...
extern int line_no;
extern int state;
// removes comments from the code in 'testcasefile' and writes the cleaned code
// to 'cleanFile'
...
char *get_name() {
  ...
  char *lexeme = (char *)malloc(maxlen); // allocates memory to store lexeme name
...
// This function basically fills the twin buffer and returns character to check
// on dfa
...
// the basic function which simulates the dfa and returns the token to syntax
// analyser
tkn getNextToken(FILE *fp) {
  ...
  while (true) {
    ...
    case 8:;
      ...
      if (strcmp(search_result, "\0") == 0) { // not found in lookup table
    ...
    case 11:;
      ...
      if (ln < 2 || ln > 20) {
        token.name = "ERROR";
        if (ln > 20)
          token.lexeme_value = "Variable Identifier is longer than the "
                               "prescribed length of 20 characters.";
        if (ln < 2)
          token.lexeme_value = "Variable Identifier is shorter than the "
                               "prescribed length of 2 characters.";
        lexemeBegin = (forward) % (BUFFER_SIZE);
        return token;
      }
...
void printTokens(char *testfile) {
  ...
  while (buffer[forward] != EOF) {
    ...
    if (strcmp(tok.name, "ERROR") == 0) {
      printf("\033[1;34mLine no. %d \033[0m", tok.line);
